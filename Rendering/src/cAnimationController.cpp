...
void UQTtoUDQ(XMFLOAT4* dual, XMFLOAT4 q, XMFLOAT4& tran ) {
    /*dual[0].x = q.x ; 
    dual[0].y = q.y ; 
    dual[0].z = q.z ; 
    dual[0].w = q.w ; 
...
void SceneAnimator::CalcBoneMatrices(){
	for( size_t a = 0; a < Transforms.size(); ++a){
		XMMATRIX result = XMLoadFloat4x4(&Bones[a]->Offset) * XMLoadFloat4x4(&Bones[a]->GlobalTransform);
		XMStoreFloat4x4(&Transforms[a] , result); // [CUSTOM]
		/*
...
cBone* SceneAnimator::CreateBoneTree( aiNode* pNode, cBone* pParent){
	...
	XMMATRIX mat = XMLoadFloat4x4(&internalNode->LocalTransform);	// [CUSTOM]
	XMStoreFloat4x4(&internalNode->LocalTransform, XMMatrixTranspose(mat));
